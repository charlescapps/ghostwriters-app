...
function scene:create(event)
    ...
    rack = rack_class.new(gameModel, 100, display.contentWidth + 220, 7, 25, board)
...
completeMove = function(move)
    if move.moveType == common_api.GRAB_TILES then
        print ("Completing a Grab Tiles move...")
        rack:addTiles(move.tiles)
        board:complete_grab()
    elseif move.moveType == common_api.PLAY_TILES then
        print ("Completing a Play Tiles move...")
        rack:completePlay()
        board:playTiles()
    end
end
