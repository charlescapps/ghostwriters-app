...
scene.sceneName = "scenes.my_offered_games_scene"
...
function scene:show( event )
    ...
    if ( phase == "will" ) then
        ...
        self.myChallengersView = my_challengers_view_class.new(user, false, self)

        common_api.getGamesOfferedByMe(common_api.MAX_GAMES_IN_PROGRESS, self:getOnSuccessCallback(), self:getOnFailCallback(), self:getOnFailCallback(), true)
...
function scene:createGoToOfferedGamesButton()
    local button = widget.newButton( {
        ...
        width = 200,
        height = 150,
        defaultFile = "images/my_challengers_button_default.png",
        overFile = "images/my_challengers_button_over.png",
        onRelease = self:getOnReleaseGoToMyChallengersButton()
    } )
    return button
end

function scene:getOnReleaseGoToMyChallengersButton()
    return function(event)
        nav.goToSceneFrom(self.sceneName, "scenes.my_challengers_scene")
    end
end
