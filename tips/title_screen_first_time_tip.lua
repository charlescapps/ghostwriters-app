...
local TIP_NAME = "title_screen_first_time_tip"
...
function M:triggerTipOnCondition()
    ...
    local singlePlayerButton = self.scene.buttonSinglePlayer
    if not common_ui.isValidDisplayObj(singlePlayerButton) then
        return false
    end

    return self:showTip(sceneView, singlePlayerButton)
end

function M:showTip(sceneView, singlePlayerButton)
    if not tips_persist.isTipViewed(TIP_NAME) then

        local shiny = shiny_tutorial_widget.new {
            tipText = "Tap here\nto start your\nfirst game!",
            focusedDisplayObj = singlePlayerButton,
            sceneView = sceneView,
